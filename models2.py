...
class TestVAE(nn.Module):
    def __init__(self, shape):
        ...
        self.encoder = nn.Sequential(
         ...
         # Latent space
        self.mean_layer = nn.Linear(6,6)#nn.Conv2d(in_channels=adj_var*1, out_channels=adj_var, kernel_size=3, stride=1, padding=0)#nn.Linear(6*6,  6*6)
        self.logvar_layer = nn.Linear(6,6)#nn.Conv2d(in_channels=adj_var*1, out_channels=adj_var, kernel_size=3, stride=1, padding=0)#nn.Linear(6*6 ,  6*6)
...
def train_model(train_dataloader, test_dataloader, signal_dataloader, model, loss_fn, optimizer, epochs, batch_size):
    for epoch in tqdm(range(epochs)):
        ...
        test_loss, test_lx, test_ly =(eval_loop(test_dataloader, model, loss_fn, test=True, signal=False)) 
        signal_loss, signal_lx, signal_ly = (eval_loop(signal_dataloader, model, loss_fn, test=False, signal=True))
