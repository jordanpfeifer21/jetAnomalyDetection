...
def run(dataset):
    ...
    in_channels, out_channels, lr, n_epochs = dataset.num_features, 16, 1e-2, 20
...
graphs_signal = graph_data_loader([signal])
dataset = JetDataset(graphs, transform = transform)
dataset_signal = JetDataset(graphs_signal, transform=transform)
run(dataset) 
