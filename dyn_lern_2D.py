...
from tensorflow.keras.layers import Input, Conv2D, Conv2D, MaxPooling2D, Flatten, Dense, Reshape, UpSampling2D, AveragePooling2D, Conv2DTranspose
...
def plot_property_distribution(data, anomaly_data, properties):
    for i in range(len(properties)):
        ...
        plt.savefig('/isilon/export/home/rpankaj/jetAD/jetAnomalyDetection/Ekin/Figures/Distribution of ' + str(prop))
        plt.clf()
...
class Autoencoder(): 
    ...
    def build_model(self):  
        ...
        x = Conv2D(5, kernel_size=(4,4), padding='same', activation=lambda x: relu(x, alpha=alpha_init))(x)
        x = Flatten()(x)
        x = Dense(self.hp_units, activation=lambda x: relu(x, alpha=alpha_init))(x)
        encoded = Dense(32)(x)
    ...
    def train(self, train_data, test_data, epochs): 
        ...
        checkpoint_callback = ModelCheckpoint(filepath='/isilon/export/home/rpankaj/jetAD/jetAnomalyDetection/Ekin/Weights/weights_epoch_{epoch:02d}.h5', save_freq='epoch', period=5)
        self.history = self.model.fit(train_data, train_data, epochs=epochs, batch_size=5000, validation_data=(test_data, test_data), callbacks=[early_stopping, checkpoint_callback, lr_scheduler_callback])

        self.train_loss = self.history.history['loss']
        self.val_loss = self.history.history['val_loss']

        self.model.save('/isilon/export/home/rpankaj/jetAD/jetAnomalyDetection/Ekin')
    ...
    def plot_loss(self): 
        ...
        plt.savefig('/isilon/export/home/rpankaj/jetAD/jetAnomalyDetection/Ekin/Figures/Training_Validation_Losses')
        plt.clf()
    ...
    def plot_anomaly_score_distribution(self):
        ...
        plt.legend()
        plt.savefig('/isilon/export/home/rpankaj/jetAD/jetAnomalyDetection/Ekin/Figures/Anomaly_Score_Distribution')
        plt.clf()
    ...
    def plot_example(self, test_data, properties, title):
        ...
        for j in range(len(properties)): 
            ...
            for i in range(n_files): 
                weight_file = '/isilon/export/home/rpankaj/jetAD/jetAnomalyDetection/Ekin/Weights/weights_epoch_{:02d}.h5'.format((i + 1) * 5)
            ...
            plt.tight_layout()
            plt.savefig("/isilon/export/home/rpankaj/jetAD/jetAnomalyDetection/Figures/RECO_Example" + properties[j] + ' ' + title)
            plt.show()
            plt.clf()
...
wjet_file = '/isilon/export/home/rpankaj/jetAD/data/Ekin/output_array-HtoBB-61440-dz-pT.npy'
...
#plot_example(self, test_data, properties, title)
Collapse
